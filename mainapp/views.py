...
def contact(request):
     if request.method == 'POST':
        ...
        messages.success(request,"your message is have been submitted successfully")
...
def log(request):
    if request.method == 'POST':
        ...
        password = request.POST.get('password')
        try:
            login = LoginInfo.objects.get(username=username,password=password)
            if login is not None:
                if login.usertype.lower() == "homeowner":
                    request.session['homeownerid']= username
                    messages.success(request,"welcome homeowner")
                    return redirect('homeownerdash')
                elif login.usertype.lower() == "contractor":
